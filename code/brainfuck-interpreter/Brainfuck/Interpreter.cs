    ...
    public static void Execute(string code)
    {
        ...
        for (int pc = 0; pc < prog.Length; pc++)
        {
            ...
            if (prog[pc] == '[')
            {
                ...
                if (currentCell.Data == 0) // Current cell of 0 at loop start [ means skip loop, so we need to find the closing brace
                {
                    ...
                    for (int i = pc + 1; i < prog.Length; i++) // Add 1 to pc for starting point to skip initial [
                    {
                        if (prog[i] == '[')
                        {
